		...
		<div className="relative w-full overflow-hidden ">
							...
							<div className="mx-auto flex w-full max-w-[1400px] flex-col-reverse items-center justify-between gap-6 px-4 py-10 md:flex-row md:gap-10 md:px-12 md:py-0">
								...
								<div className="z-10 w-full shrink-0 self-center rounded-[12px] bg-white px-6 py-6 shadow-md md:w-[460px] md:px-8 md:py-8">
									<div className="mb-2 flex items-start gap-4 md:mb-4">
										<span className="mt-2 block h-[2px] w-6 shrink-0 bg-mainHover md:mt-3 md:w-8" />
										<div className="flex-1">
											{title && (
												<h2 className="mb-2 text-2xl font-extrabold leading-tight text-button md:text-3xl">
													{title}
												</h2>
											)}
											{description && (
			...
			{count > 1 && (
				<>
					<button
						onClick={prev}
						className="absolute left-2 top-1/2 z-20 flex h-16 w-10 -translate-y-1/2 items-center justify-center text-main transition hover:text-mainHover md:left-4"
						aria-label="Précédent"
					>
						<Icon id="chevron-left" className="h-10 w-6" />
					</button>
					<button
						onClick={next}
						className="absolute right-2 top-1/2 z-20 flex h-16 w-10 -translate-y-1/2 items-center justify-center text-main transition hover:text-mainHover md:right-4"
						aria-label="Suivant"
					>
						<Icon id="chevron-right" className="h-10 w-6" />
					</button>
				</>
			)}
