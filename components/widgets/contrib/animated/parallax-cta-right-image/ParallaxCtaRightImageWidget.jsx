...
const ParallaxCtaRightImage = ({ title, content, image, cta }) => {
	...
	return (
		<div className="relative md:h-[50vh]">
			<motion.div
				...
				className="mr-auto flex flex-col items-start p-10 md:w-6/12 md:pr-16"
				ref={containerRef}
			>
				<motion.h2
			...
			<div className="z-1 relative right-0 top-0 h-96 overflow-hidden md:absolute md:h-[50vh] md:w-6/12">
				<motion.img
			...
			{!isMobile && (
				<motion.div
					style={{ y: background }}
					className="absolute bottom-0 left-0 right-0 z-[-100] mx-auto h-64 w-5/12 rounded-l-3xl bg-gray-50"
				></motion.div>
			)}
		</div>
	)
...
const ParallaxCtaRightImageContainer = ({ data }) => {
	const widgetData = {
	...
	return <ParallaxCtaRightImage {...widgetData} />
}

export default ParallaxCtaRightImageContainer
